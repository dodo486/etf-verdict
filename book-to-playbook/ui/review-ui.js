...
(function(){
  ...
  function jump(key, title, targetLabel){
    ...
    var prod = REF_PROD[key];   // ref→종목은 rules.json 에서만 파생(책 하드코딩 없음). 없으면 칩을 안 연다 — 그런 소절의 착지 대상은 칩 밖 정적 블록뿐이다.
    if(prod){
...
(function(){
  ...
  // 장 헤더 라벨은 그 책이 #src 에 쓴 제 헤딩에서 뽑는다(책-무관: 'Chapter'/'N장' 등 문구를 코드가 정하지 않음).
  var CHAP_TITLE = {};
  try {
    document.getElementById('src').textContent.split('\n').forEach(function(l){
      var mm = l.match(/^##\s+([0-9A-Za-z가-힣]+)\.\s*(.*)$/);
      if(mm){ CHAP_TITLE[mm[1]] = (mm[1] + '. ' + mm[2]).trim(); }
    });
  } catch(e){}
  ...
  function render(filter){
    ...
    keys.forEach(function(k){
      ...
      var chapKey = k === '에필로그' ? '에필로그' : k.split('-')[0];
      var chap = CHAP_TITLE[chapKey] || (chapKey === '에필로그' ? '에필로그' : chapKey + '장');
